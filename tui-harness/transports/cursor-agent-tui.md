...
- **既定モード（agent）で起動する**: `--mode ask` は読み取り専用のため回答ファイルの書き出しと完了通知が行えない。agent モードは書き込みを無確認で通す（防御の正本は ../SKILL.md「呼び出しパラメータ」の裁定スコープ項）。
- ../SKILL.md「委譲先の確定と検証」で確定したモデルを `--model <モデル>` で必ず付加する（実効モデルは起動後にペインのフッターへ表示される）。
